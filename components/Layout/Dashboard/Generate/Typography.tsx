...
export function TypographyH1({ children }: { children: ReactNode }) {
  return (
    <h1 className="mb-3.5 w-full scroll-m-20 text-center text-4xl font-extrabold tracking-tight text-balance">
      {children}
    </h1>
  );
}
...
export function TypographyPre({ children }: { children: ReactNode }) {
  return (
    <pre className="h-fit w-full overflow-auto rounded-md bg-stone-100 p-2.5" id={'hide-scrollbar'}>
      {children}
    </pre>
  );
}
